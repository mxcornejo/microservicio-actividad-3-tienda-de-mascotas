...
public class DataService {
	...
	// Productos
	public Product agregarProducto(String nombre, String categoria, double costo, double precio) {
		...
		Product p = new Product(productIdCounter++, nombre, categoria, costo, precio);
		products.add(p);
		return p;
	}
	...
	public Customer agregarCliente(String nombre, String email) {
		if (nombre == null || nombre.isBlank()) throw new IllegalArgumentException("El nombre del cliente no puede estar vacío.");
	...
	// Ventas
	public Sale registrarVenta(int productId, int customerId, int cantidad, double precioUnitario, LocalDate fecha) {
		if (cantidad <= 0) throw new IllegalArgumentException("La cantidad debe ser mayor a 0.");
		if (precioUnitario <= 0) throw new IllegalArgumentException("El precio unitario debe ser mayor a 0.");
		if (fecha == null) throw new IllegalArgumentException("La fecha es obligatoria.");

		Product producto = buscarProducto(productId).orElseThrow(() -> new IllegalArgumentException("Producto no existe."));
		Customer cliente = buscarCliente(customerId).orElseThrow(() -> new IllegalArgumentException("Cliente no existe."));
		// Nota: no forzamos que precioUnitario == producto.getPrice(), pero se podría validar si el negocio lo requiere.

		Sale nueva = new Sale(saleIdCounter++, producto.getId(), cliente.getId(), cantidad, fecha, precioUnitario);
		sales.add(nueva);
		return nueva;
	}
	...
	public List<Sale> getSalesByRange(LocalDate desde, LocalDate hasta) {
		if (desde == null || hasta == null) throw new IllegalArgumentException("Fechas 'from' y 'to' son obligatorias.");
		return sales.stream()
	...
	public double gananciasPorMes(int anio, int mes) {
		...
		validarMes(mes);
		return sales.stream()
				.filter(v -> v.getDate().getYear() == anio && v.getDate().getMonthValue() == mes)
				.mapToDouble(v -> v.getUnitPrice() * v.getQuantity())
				.sum();
	}
	...
	public ProfitSummary resumenPorDia(LocalDate fecha, String moneda) {
		...
		return construirResumen(
				"Día " + fecha,
				moneda,
				sales.stream().filter(v -> fecha.equals(v.getDate())).collect(Collectors.toList())
		);
	}
	...
	public ProfitSummary resumenPorMes(int anio, int mes, String moneda) {
		...
		return construirResumen(
				...
				moneda,
				sales.stream().filter(v -> v.getDate().getYear() == anio && v.getDate().getMonthValue() == mes).collect(Collectors.toList())
		);
	}
	...
	public ProfitSummary resumenPorAnio(int anio, String moneda) {
		...
		return construirResumen(
				String.valueOf(anio),
				moneda,
				sales.stream().filter(v -> v.getDate().getYear() == anio).collect(Collectors.toList())
		);
	}

	private ProfitSummary construirResumen(String periodo, String moneda, List<Sale> ventasPeriodo) {
		double ingresos = ventasPeriodo.stream()
				.mapToDouble(v -> v.getUnitPrice() * v.getQuantity())
				.sum();

		// Costo total basado en el costo del producto
		Map<Integer, Product> productIndex = products.stream().collect(Collectors.toMap(Product::getId, p -> p));
		...
		return new ProfitSummary(periodo, moneda, ingresos, costo, ventasPeriodo.size());
	}
	...
	private void validarMes(int mes) {
		if (mes < 1 || mes > 12) throw new IllegalArgumentException("Mes inválido (1-12).");
	}
	...
	public Map<String, Integer> salesUnitsByCategory(LocalDate from, LocalDate to) {
		if (from == null || to == null) throw new IllegalArgumentException("Fechas 'from' y 'to' son obligatorias.");
		var productIndex = products.stream().collect(Collectors.toMap(Product::getId, p -> p));
